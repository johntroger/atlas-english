...
- Started VS-01: initialized the local Git repository and the minimum Next.js/TypeScript modular-monolith foundation, generated an exact npm lockfile, added immutable-SHA CI plus formatting, lint, architecture-boundary, type, unit, secret and production-build checks, and verified the foundation at desktop and mobile viewports. GitHub remote publication and remote CI remain pending; Supabase and Vercel are not connected.
- Published the VS-01 foundation to the public `johntroger/atlas-english` repository; after the first Windows CI run exposed CRLF conversion in `format:check`, added repository-level LF normalization and a regression test before rerunning CI.
