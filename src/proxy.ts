...
async function valid(value: string | undefined): Promise<boolean> {
  ...
  if (!expiryText || !received || extra || Number(expiryText) <= Date.now()) return false;
  ...
  const signed = await crypto.subtle.sign(
    "HMAC",
    key,
    new TextEncoder().encode(`alpha:${expiryText}`),
  );
...
export async function proxy(request: NextRequest) {
  ...
  if (
    request.nextUrl.pathname.startsWith("/alpha-access") ||
    request.nextUrl.pathname.startsWith("/api/alpha-access")
  )
    return NextResponse.next();
  if (!process.env.ATLAS_ALPHA_ACCESS_SECRET)
    return new NextResponse("Owner Alpha is not configured.", { status: 503 });
